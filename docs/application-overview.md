# NextWork Application Overview

> **Doc revision:** v113  
> Last synchronized: 2026-05-11 (voice: hiring marketplace profesional; CTA kontak; lihat `features.md`).

Dokumen ini menjelaskan gambaran umum aplikasi NextWork: tujuan produk, area fitur, arsitektur singkat, dan peta route utama.

> Ringkasan “**produk ini apa**” untuk non-engineer: **`docs/apa-itu-nextwork.md`**.

## Update status (April 2026)

- **Professional voice / i18n (2026-05-11):** kamus memperjelas kontak via Pesan, mengganti CTA “start discussion”, dan merapikan bahasa Indonesia formal pada label onboarding, hero, dan job—tanpa mengubah route atau layout.
- **Public copy tone (2026-05-11):** teks pengguna di beranda pemasaran dan board publik (`/jobs`, `/freelancers`) memakai kamus EN/ID yang lebih ringkas dan operasional—judul section, empty state, dan trust line—tanpa mengubah layout atau data backend.
...
- **E2E HTTP harness (2026-05-09):** `pnpm test:e2e` memakai `scripts/run-e2e-server.mjs` untuk build `@acme/web`, menjalankan **`next start`** pada **`127.0.0.1:3041`** (override `E2E_PORT`), meng-set `BASE_URL`, lalu menjalankan `scripts/e2e-marketplace-flow.mjs`; mengurangi kegagalan palsu akibat chunk webpack hilang saat API dipanggil ke `next dev` dengan `.next` inkremental rusak. **2026-05-12:** bila **`DATABASE_URL_TEST`** diset, runner memetakan ke `DATABASE_URL` untuk build + server agar smoke tidak menulis ke DB staging publik secara tidak sengaja. Runner memuat **`.env`**, **`.env.local`** (root), **`apps/web/.env.local`** bila kunci belum ada di lingkungan.
- **Public homepage marketplace pass (2026-05-09):** landing `/[locale]` memperjelas positioning marketplace terstruktur (lokal/remote/hybrid, alur lowongan → proposal → chat → hire), CTA terpisah klien (`Post a job`) vs freelancer (`Browse jobs`), formulir pencarian memakai kategori nyata dari layanan kategori + filter `workMode`, panel kanan/mobile diganti **kartu alur proses** (bukan profil contoh), serta section tambahan “How NextWork works”, manfaat dua audiens, pesan trust/safety tanpa metrik palsu, dan early-access gratis.
- **Locale persistence navigation (2026-05-09):** tautan internal ke marketplace SEO roots (`jobs`, `freelancers`, `how-it-works`, `pricing`, `early-access`, `help`) dan banyak dashboard/workspace surfaces memakai **`withPublicLocale`** / **`withWorkspaceLocale`** (plus middleware **`resolveNavigationLocale`** untuk path tidak ber-prefix) sehingga sesi di **`/en/…`** tidak “meloncat” ke **`/id/…`** hanya karena cookie **`lang`** tidak sama dengan locale aktif di pathname atau referer.
- **Workspace locale URLs (2026-05-10):** pengguna melihat path berbahasa konsisten di bilah alamat: segment workspace memakai awalan `en` atau `id`, disinkronkan dengan cookie `lang` dan header `x-nextwork-locale`. Implementasi memakai `NextResponse.rewrite` ke handler route yang sudah ada; link di nav dashboard/marketing memakai helper `apps/web/lib/i18n/workspace-path.ts`.
...
- **Localized marketing chrome (2026-05-10):** halaman beranda bertitel `/en`/`/id` merender hero pemasaran dari kamus aktif (`RootLayout` + header `x-nextwork-locale`); tidak ada lagi string tetap Bahasa Indonesia di navbar/footer/chips cepat untuk pengguna bahasa Inggris.
...
NextWork adalah platform marketplace kerja freelance yang menghubungkan:
