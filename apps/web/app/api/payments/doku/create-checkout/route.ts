...
export async function POST(request: Request) {
  return withApiHandler(async () => {
    ...
    const user = await db.user.findUnique({ where: { id: gate.actor.userId }, select: { email: true, clientProfile: { select: { displayName: true } } } });
    return jsonOk(await service.createContractCheckout({ userId: gate.actor.userId, contractId: parsed.data.contractId, clientEmail: user?.email ?? "client@dnpeople.local", clientName: user?.clientProfile?.displayName ?? "Client" }));
  });
}
