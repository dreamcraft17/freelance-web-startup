...
export function LandingProductPreview() {
  return (
    <section className="mx-auto max-w-6xl px-4 pb-4 pt-2 sm:px-6 sm:pb-6">
      ...
      <ul className="nw-surface mt-4 divide-y divide-slate-200 overflow-hidden">
        {examples.map((row) => (
          <li key={row.title} className="flex flex-col gap-2 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:gap-6 sm:px-5 sm:py-5">
            <div className="min-w-0 flex-1">
              <div className="mb-1 flex flex-wrap items-center gap-2">
                <span
                  className={
                    row.kind === "Job"
                      ? "rounded-md bg-amber-100/90 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-amber-900"
                      : "rounded-md bg-indigo-100/90 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-indigo-900"
                  }
                >
                  {row.kind}
                </span>
            ...
            <div className="flex shrink-0 flex-col items-start gap-1.5 sm:items-end sm:text-right">
              <span className="inline-flex items-center gap-1.5 rounded-md border border-indigo-100 bg-indigo-50/90 px-2 py-1 text-xs font-medium text-indigo-950">
