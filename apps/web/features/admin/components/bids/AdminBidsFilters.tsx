...
const STATUS_OPTIONS = Object.values(BidStatus).sort((a, b) => a.localeCompare(b));

type AdminBidsFiltersProps = {
  status?: BidStatus;
  q?: string;
  freelancer?: string;
};

/** GET form — filters bids by status, job title, and freelancer username. */
export function AdminBidsFilters({ status, q, freelancer }: AdminBidsFiltersProps) {
  return (
    <form
      method="get"
      action="/admin/bids"
    ...
    >
      <label className="flex min-w-[10rem] flex-1 flex-col gap-1">
        <span className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Bid status</span>
        <select
          name="status"
          defaultValue={status ?? ""}
      ...
      <label className="flex min-w-[11rem] flex-1 flex-col gap-1">
        <span className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Freelancer username</span>
        <input
          type="search"
          name="freelancer"
