...
export function FreelancersBrowseList({ freelancers, activeCityFilter }: FreelancersBrowseListProps) {
  ...
  const compareSignals = (f: PublicFreelancerCard): string[] => {
    const signals: string[] = [];
    if (f.availabilityStatus === "AVAILABLE") {
      signals.push(t("public.freelancers.signalAvailableWeek"));
    }
    if (f.distanceKm != null && Number.isFinite(f.distanceKm) && f.distanceKm <= 25) {
      signals.push(t("public.freelancers.signalNearby"));
    }
    if (f.reviewCount >= 6 && (f.averageReviewRating ?? 0) >= 4.7) {
      signals.push(t("public.freelancers.signalTopRatedLocal"));
    } else if (f.reviewCount >= 3 && (f.averageReviewRating ?? 0) >= 4.5) {
      signals.push(t("public.freelancers.signalPopularChoice"));
    }
    const ageDays = daysSince(f.createdAt);
  ...
  return (
    <ul className="space-y-3">
      {freelancers.map((f) => {
        ...
        const signals = compareSignals(f);
